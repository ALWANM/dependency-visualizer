...
function replaceParentFolderPlaceholder(
  ...
  // Remove root path first
  // let newRefPath = removeRootPath(refPath, rootPath);

  // // Split parent path to get ancestor directories
  // const parentDirectories = path.dirname(parentPath).split(path.sep);
  // logDebugMessage(`parentDirectories :  ${parentDirectories} .`);
  // // Count the number of ../ patterns
  // const ancestorCount = (newRefPath.match(/^\.\./g) || []).length;
  // logDebugMessage(`ancestorCount :  ${ancestorCount} .`);
  // // Find the position of the last backslash in the filePath
  // const lastBackslashIndex = newRefPath.lastIndexOf(".\\");
  // logDebugMessage(`lastBackslashIndex :  ${lastBackslashIndex} .`);
  // for (let i = 0; i < ancestorCount; i++) {
  //   if (parentDirectories.length - 3 - i >= 0) {
  //     newRefPath = `${
  //       parentDirectories[parentDirectories.length - 3 - i]
  //     }\\${newRefPath.substring(lastBackslashIndex + 2)}`;
  ...
  // newRefPath = newRefPath.replace(/..\//g, "");
